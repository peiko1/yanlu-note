package com.peikol.xiaohashu.comment.biz.service.impl;
...
import com.peikol.framework.common.constant.DateConstants;
...
import com.peikol.framework.common.util.DateUtils;
import com.peikol.framework.common.util.JsonUtils;
import com.peikol.xiaohashu.comment.biz.constant.MQConstants;
import com.peikol.xiaohashu.comment.biz.constant.RedisKeyConstants;
import com.peikol.xiaohashu.comment.biz.domain.dataobject.CommentDO;
import com.peikol.xiaohashu.comment.biz.domain.dataobject.CommentLikeDO;
import com.peikol.xiaohashu.comment.biz.domain.mapper.CommentDOMapper;
import com.peikol.xiaohashu.comment.biz.domain.mapper.CommentLikeDOMapper;
import com.peikol.xiaohashu.comment.biz.domain.mapper.NoteCountDOMapper;
import com.peikol.xiaohashu.comment.biz.enums.*;
import com.peikol.xiaohashu.comment.biz.model.dto.LikeUnlikeCommentMqDTO;
import com.peikol.xiaohashu.comment.biz.model.dto.PublishCommentMqDTO;
import com.peikol.xiaohashu.comment.biz.model.vo.*;
import com.peikol.xiaohashu.comment.biz.retry.SendMqRetryHelper;
import com.peikol.xiaohashu.comment.biz.rpc.DistributedIdGeneratorRpcService;
import com.peikol.xiaohashu.comment.biz.rpc.KeyValueRpcService;
import com.peikol.xiaohashu.comment.biz.rpc.UserRpcService;
import com.peikol.xiaohashu.comment.biz.service.CommentService;
import com.peikol.xiaohashu.kv.dto.req.FindCommentContentReqDTO;
...
public class CommentServiceImpl implements CommentService {
    ...
    public PageResponse<FindCommentItemRspVO> findCommentPageList(FindCommentPageListReqVO findCommentPageListReqVO) {
        ...
        String commentTotal = (String) redisTemplate.opsForHash()
                .get(noteCommentTotalKey, RedisKeyConstants.FIELD_COMMENT_TOTAL);

        // 若缓存不存在，则查询数据库
        Long count = StringUtils.isBlank(commentTotal) ? 0L : Long.parseLong(commentTotal);
        if (StringUtils.isBlank(commentTotal)) {
            // 查询评论总数 (从 t_note_count 笔记计数表查，提升查询性能, 避免 count(*))
            count = noteCountDOMapper.selectCommentTotalByNoteId(noteId);

            // 异步将评论总数同步到 Redis 中
            Long finalCount = Optional.ofNullable(count).orElse(0L);
            threadPoolTaskExecutor.execute(() ->
                    redisTemplate.opsForHash()
                            .put(noteCommentTotalKey, RedisKeyConstants.FIELD_COMMENT_TOTAL, String.valueOf(finalCount)));
        }
